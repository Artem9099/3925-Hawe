...
(function (TcHmi) {
  ...
  var PopupOpen = function PopupOpen(Content, Header, HorizontalAlignment, VerticalAlignment, HorizontalDistance, VerticalDistance, Height, Width, WhereToInsert, AddWhereClicked) {
    ...
    if (popupType == "usercontrol") {
      ...
      for (var b = 0; b < PopParameters.length; b++) {
        if (PopParameters[b].symbolExpression.search("%pp%") > -1) {
          ...
          if (originControl) {
            var symbolExpression = TcHmi.Binding.resolveEx('stParams', originControl);
            if (symbolExpression) {
              if (symbolExpression.toString().toLowerCase().search("stparams") > -1) {
